...
class Document {
public:
    // by setting it equal to zero, this function becomes a pure virtual function = 0 = pure specifier
    // This forces the child class to implement this funcion
...
int main() {
	XML xml;
    // Document Serialize is called! not the one from XML!
    // Child classes of Document should be FORCED to override the serialize function
    // Now the Serialize function is implemented in XML, this one is called
	Write(&xml);

    // an abstract cannot be instantiated, but can be used through a ptr or reference
    // used for creating an interface
